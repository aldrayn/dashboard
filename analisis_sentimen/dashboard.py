...
ori_df = pd.read_csv(f'{script_dir}/dataset/dataset_youtube-comments-scraper_2024-07-30_03-58-00-919.csv')

data = pd.read_csv(f'{script_dir}/dataset/clean_dataset.csv', index_col = 0)
data = data[data['sentimen'] != 'Netral']

positif_df = pd.read_csv(f'{script_dir}/dataset/positif.csv', index_col = 0)
negatif_df = pd.read_csv(f'{script_dir}/dataset/negatif.csv', index_col = 0)
...
# Mencari K-Terbaik di antara 10-199 Features
k_feature_acc = 0
k_feature_f1 = 0
max_acc = 0
max_f1 = 0
...
# chi square 86 fitur
chi2_86_features = SelectKBest(chi2, k=86)
X_86_best_features = chi2_86_features.fit_transform(train_tf_idf, y_train)

nb86 = GaussianNB()
nb86.fit(X_86_best_features, y_train)

X_test_86_chi2 = chi2_86_features.transform(X_test_TFIDF)

y_pred_86 = nb86.predict(X_test_86_chi2)

kolom_86 = ['negatif', 'positif']
confm_86 = confusion_matrix(y_test, y_pred_86)
df_cm_86 = pd.DataFrame(confm_86, index = kolom_86, columns = kolom_86)
...
if menu == 'Introduction' :
    ...
    with col1 :
        st.write('**Data Kata Positif**')
        st.dataframe(positif_df, hide_index = True, use_container_width = True)
    
    with col2 :
        st.write('**Data Kata Negatif**')
        st.dataframe(negatif_df, hide_index = True, use_container_width = True)
    ...
    if selected_option == options[1] :
        ...
        with st.container() :
            ...
            st.markdown("<h4 style='text-align: left;'>Confusion Matrix</h4>", unsafe_allow_html=True)
            sns.heatmap(df_cm_86, cmap = 'Greens', annot=True, fmt=".0f", ax = ax)
            ax.set_xlabel('Sentimen Sebenarnya')
            ax.set_ylabel('Sentimen Prediksi')
            ...
            st.markdown("<h4 style='text-align: left;'>Classification Report</h4>", unsafe_allow_html=True)
            st.text(classification_report(y_test, y_pred_86))
